...
class DiffDriveGUI:
    def __init__(self, root: tk.Tk, default_port: str = "/dev/ttyUSB0"):
        ...
        # FIX: Track last CMD_VEL send time for adaptive sending
    ...
    def _build_ui(self, default_port: str):
        ...
        # Add auto-stream checkbox
        self._auto_stream_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(top, text="Auto-stream", variable=self._auto_stream_var).pack(side=tk.LEFT, padx=3)
    ...
    def _build_right(self, parent):
        ...
        # FIX: Updated default PID values to match firmware (Right Ki = 8.0)
        motor_rows = [
            ("Left",  (10.0, 2.0, 0.1)),
            ("Right", (10.0, 8.0, 0.1)),   # Updated Ki from 5.0 to 8.0
            ("Both",  (10.0, 2.0, 0.1)),
        ]
    ...
    def _connect(self):
        ...
        try:
            ...
            for _ in range(6):
                ...
                while time.time() < deadline:
                    ...
                    if line == "READY":
                        ok = True
                        break
                    # FIX: Also check for LIMITS message during handshake
            ...
            # FIX: Auto-start streaming if checkbox is checked
            if self._auto_stream_var.get():
                self.root.after(500, self._toggle_stream)  # Delay to ensure connection is stable
        ...
        while self._running and self.ser and self.ser.is_open:
            try:
                ...
                if line.startswith("ODOM,"):
                    parts = line.split(",")
                    # FIX: Updated firmware outputs 13 fields, not 14
                    # ODOM,t_ms,l_spd,r_spd,l_pwm,r_pwm,l_sp,r_sp,v,w,x,y,yaw
                    if len(parts) == 13:  # Changed from 14 to 13
                        try:
                            odom = {
                                "t_ms":    int(parts[1]),
    ...
    def _on_slider(self):
        ...
        # FIX: Send command immediately when slider moves (while streaming)
        if self.connected and self._streaming:
            self._send(f"CMD_VEL:{self._v:.4f},{self._w:.4f}")
            self._last_cmd_time = time.time()

    # ── Periodic GUI update (main thread, 50 ms for more responsive CMD_VEL) ──

    def _poll(self):
        # FIX: Send CMD_VEL keepalive at 10 Hz when streaming
        # But only if slider hasn't changed recently (avoid duplicate sends)
        if self.connected and self._streaming:
            now = time.time()
            if now - self._last_cmd_time > 0.1:  # 100ms interval
                self._send(f"CMD_VEL:{self._v:.4f},{self._w:.4f}")
        ...
        self.root.after(50, self._poll)  # FIX: Reduced from 100ms to 50ms

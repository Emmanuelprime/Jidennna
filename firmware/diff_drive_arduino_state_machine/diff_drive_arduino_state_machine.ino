...
// Robot states
enum RobotState {
  STATE_IDLE,           // Motors stopped, waiting for command
  STATE_ACCELERATING,   // Ramping up to target speed
  STATE_CRUISING,       // Maintaining target speed
  STATE_DECELERATING,   // Ramping down to stop
  STATE_EMERGENCY_STOP, // Emergency stop state
  STATE_CALIBRATING     // IMU calibration in progress
};
...
enum StateTransition {
  ...
  TRANS_EMERGENCY,
  TRANS_CALIBRATE,
  TRANS_CALIBRATE_DONE,
  TRANS_VELOCITY_CHANGE
};

// Current state
RobotState currentState = STATE_IDLE;
StateTransition lastTransition = TRANS_NONE;
unsigned long stateStartTime = 0;

// ─── STATE MACHINE CONFIGURATION ────────────────────────────────────────────

#define ACCELERATION_RATE  0.5f   // m/s² (how fast to ramp up)
#define DECELERATION_RATE  0.8f   // m/s² (how fast to ramp down)
#define EMERGENCY_STOP_DELAY 100  // ms to hold emergency stop
#define IDLE_TIMEOUT        5000  // ms before auto-stop if no command
...
void updateStateMachine() {
  ...
  switch (currentState) {
    ...
    case STATE_ACCELERATING:
      ...
      if (abs(targetLinearVelocity - commandedLinearVelocity) < 0.01 && 
        ...
        stateStartTime = now;
        lastTransition = TRANS_VELOCITY_CHANGE;
        Serial.println("# STATE: CRUISING");
      }
      break;
    ...
    case STATE_CRUISING:
      ...
      if (commandedLinearVelocity == 0 && commandedAngularVelocity == 0) {
        ...
        stateStartTime = now;
        lastTransition = TRANS_STOP;
        Serial.println("# STATE: DECELERATING");
      }
      break;
    ...
    case STATE_DECELERATING:
      ...
      // Apply deceleration
      if (abs(decel_v) > 0) {
        if (decel_v > 0 && decel_v < 0.01) decel_v = 0;
        if (decel_v < 0 && decel_v > -0.01) decel_v = 0;
      }
      if (abs(decel_w) > 0) {
        if (decel_w > 0 && decel_w < 0.01) decel_w = 0;
        if (decel_w < 0 && decel_w > -0.01) decel_w = 0;
      }
    ...
    case STATE_EMERGENCY_STOP:
      // Emergency stop - force zero velocity
      targetLinearVelocity = 0;
      targetAngularVelocity = 0;
      
      // Apply brake (optional: reverse PWM briefly)
      if (now - stateStartTime < EMERGENCY_STOP_DELAY) {
        // Emergency braking
      }
      
      // After delay, go to idle
      if (now - stateStartTime > EMERGENCY_STOP_DELAY) {
        currentState = STATE_IDLE;
        stateStartTime = now;
        lastTransition = TRANS_EMERGENCY;
        Serial.println("# STATE: IDLE (after emergency stop)");
      }
      break;
    ...
    case STATE_CALIBRATING:
      ...
      if (now - stateStartTime > 3000) {
        currentState = STATE_IDLE;
        stateStartTime = now;
        lastTransition = TRANS_CALIBRATE_DONE;
        Serial.println("# STATE: IDLE (calibration complete)");
      }
      break;
  }
  ...
  if (currentState != STATE_IDLE && currentState != STATE_EMERGENCY_STOP) {
    ...
    if (now - lastCommandTime > IDLE_TIMEOUT && lastCommandTime > 0) {
...
void setCommand(float v, float w) {
  ...
  // Reset state machine if coming from idle
...
void setup() {
  ...
  Serial.println("# State Machine Enabled");
  Serial.println("# Commands:");
  Serial.println("#  V<v>,<w>    - Set velocity (smooth acceleration)");
  ...
  Serial.println("#  s           - Stop (smooth deceleration)");
  ...
  if (Serial.available()) {
    ...
    if (cmd == "PING") {
      ...
      switch (currentState) {
        ...
        case STATE_ACCELERATING: Serial.println("ACCELERATING"); break;
        case STATE_CRUISING: Serial.println("CRUISING"); break;
        case STATE_DECELERATING: Serial.println("DECELERATING"); break;
      ...
      Serial.print(targetLinearVelocity, 3);
      Serial.print(", w=");
      Serial.println(targetAngularVelocity, 3);
      
    } else if (cmd == "s" || cmd == "S") {
      // Smooth stop
      commandedLinearVelocity = 0;
      commandedAngularVelocity = 0;
      if (currentState == STATE_CRUISING || currentState == STATE_ACCELERATING) {
        currentState = STATE_DECELERATING;
        stateStartTime = millis();
        lastTransition = TRANS_STOP;
        Serial.println("# STATE: DECELERATING");
      ...
      stateStartTime = millis();
      lastTransition = TRANS_CALIBRATE;
      mpu6050.calcGyroOffsets(true);
      mpu6050.update();
      currentYaw = mpu6050.getAngleZ();
      filteredYaw = currentYaw;
      ...
      leftPrevError = 0;
      rightPrevError = 0;
      Serial.print("# Command: Forward v=");
      ...
      leftPrevError = 0;
      rightPrevError = 0;
      Serial.print("# Command: Reverse v=");
      Serial.println(speed, 3);
      ...
      leftPrevError = 0;
      rightPrevError = 0;
      Serial.print("# Command: Spin left omega=");
      ...
      leftPrevError = 0;
      rightPrevError = 0;
      Serial.print("# Command: Spin right omega=");
      Serial.println(omega, 3);
